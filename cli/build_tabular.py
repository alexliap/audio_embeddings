...
def get_data_from_source(data_dir: str) -> dict:
    data = {
        "work": [],
        "performance": [],
        "chroma_cens": np.empty([12,]),
        "crema": np.empty([12,]),
        "hpcp": np.empty([12,]),
        "mfcc_htk": np.empty([13,]),
        "novfn": [],
        "snovfn": [],
        "tempo": [],
    }
    
    for work in tqdm(os.listdir(data_dir), bar_format='{l_bar}{bar:20}{r_bar}{bar:-20b}'):
        if work in [".DS_Store", "README.md", "LICENSE.txt"]:
            ...
            for perf in os.listdir(os.path.join(data_dir, work)):
                ...
                with h5py.File(file, "r") as f:
                    ...
                    crema = np.array(f['crema']).mean(axis=1)
...
if __name__ == "__main__":
    ...
    data = data.with_columns(pl.col("chroma_cens").list.to_struct(fields=lambda idx: f"chroma_cens_{idx}")).unnest('chroma_cens')
    data = data.with_columns(pl.col("crema").list.to_struct(fields=lambda idx: f"crema_{idx}")).unnest('crema')
    data = data.with_columns(pl.col("hpcp").list.to_struct(fields=lambda idx: f"hpcp_{idx}")).unnest('hpcp')
    data = data.with_columns(pl.col("mfcc_htk").list.to_struct(fields=lambda idx: f"mfcc_htk_{idx}")).unnest('mfcc_htk')
